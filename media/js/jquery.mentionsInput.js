...
(function ($, _, undefined) {
  ...
  var MentionsInput = function (input) {
    ...
    function updateValues() {
      ...
      console.log('buffer: '+settings.triggerChar);
    ...
    if(inputBuffer[0]=='@'){
      ...
      searchType=settings.triggerChar;
      var triggerCharIndex = _.lastIndexOf(inputBuffer, settings.triggerChar);
      console.log(triggerCharIndex);

      if (triggerCharIndex > -1) {
        currentDataQuery = inputBuffer.slice(triggerCharIndex + 1).join('');
        currentDataQuery = utils.rtrim(currentDataQuery);

        _.defer(_.bind(doSearch, this, currentDataQuery,searchType));
      }
    }
    ...
    function populateDropdown(query, results,searchType) {
      ...
      _.each(results, function (item, index) {
        ...
        var elmListItem = $(settings.templates.autocompleteListItem({
          ...
          'display' : utils.htmlEncode(searchType+item.username),
          'type'    : utils.htmlEncode(searchType),
    ...
    function doSearch(query,searchType) {
        console.log(query);
        //defaultSettings.triggerChar
      if (query && query.length && query.length >= settings.minChars) {

        settings.onDataRequest.call(this, 'search', query, function (responseData) {
          populateDropdown(query, responseData,searchType);
        });
      }
    }
